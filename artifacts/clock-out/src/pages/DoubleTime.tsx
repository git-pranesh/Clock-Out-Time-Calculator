...
const page = getPageByPath("/double-time-calculator")!;

export default function DoubleTime() {
  const [rate, setRate] = useState("20");
  const [dtHours, setDtHours] = useState("2");

  const r = parseFloat(rate) || 0;
  const h = parseFloat(dtHours) || 0;
  const dtRate = r * 2;
  const dtPay = dtRate * h;
  ...
  return (
    <>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-card border border-card-border rounded-xl shadow-sm overflow-hidden" data-testid="double-time-calculator">
          <div className="bg-primary px-6 py-5 text-center">
            <p className="text-primary-foreground/70 text-sm mb-1">Your Double-Time Pay</p>
            <p className="text-5xl font-bold text-white" data-testid="result-dt-pay">{fmt(dtPay)}</p>
            {r > 0 && (
              <p className="text-primary-foreground/60 text-sm mt-1" data-testid="result-dt-rate">
                {fmt(dtRate)}/hr double-time rate
              </p>
            )}
          </div>
          ...
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1.5">Double-Time Hours</label>
              <input
                type="number"
                min="0"
                step="0.5"
        ...
        <section className="bg-card border border-card-border rounded-xl p-6">
          <h2 className="text-xl font-semibold text-foreground mb-4">About Double-Time Pay</h2>
